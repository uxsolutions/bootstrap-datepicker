...
(function($, undefined){
	...
	var DPGlobal = {
		...
		parseDate: function(date, format, language){
			...
			$.each(format.separators, function (index, element) {
				if (!element) {
					return;
				}
			...
			var _date = new Date();
			_date = UTCDate(_date.getFullYear(), _date.getMonth(), _date.getDate(), 0, 0, 0);
			var start = 0;
			$.each(format.parts, function (index, element) {
				var part = date.substr(start, element.length);
				start += part.length;
				part = parseInt(part, 10);
				if (!isNaN(part)) {
					settersMap[element](_date, part);
				}
			});
